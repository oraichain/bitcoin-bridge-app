...
export class NomicClient implements NomicClientInterface {
  ...
  public get unbondingValidators(): UnbondingValidator[] {
    return this.delegations.reduce((unbondingValidators, delegation) => {
      const validator = this.validators.find(
        (validator) => validator.address === delegation.address
      );
  ...
  public async claimTestnetParticipationIncentives() {
    const data = await this.nomic.claimTestnetParticipationIncentives(
      this.wallet.address
    );
    await this.wallet.sign(data);
  ...
  public async joinRewardAccounts() {
    ...
    const evmosAirdropBalance = await this.nomic.airdropBalances(
      evmosNomicAddress
    );
    const evmosIncentiveBalances = await this.nomic.incentiveBalances(
      evmosNomicAddress
    );
    const evmosAirdropClaimedState = localStorage.getItem(
      "nomic/evmosAirdropClaimAttempted/" + this.wallet.address
    );

    // this local storage key will need to change for both testnet and mainnet
    const balanceEligible =
      evmosAirdropBalance.total() > 0n || evmosIncentiveBalances.total() > 0n;
    if (
      !balanceEligible &&
    ...
    const sig = await window.keplr.signEthereum(
      OraichainChain.chainId,
      evmosKey.bech32Address,
      data,
      EthSignType.MESSAGE
    );
  ...
  public async generateAddress(destination?: string) {
    ...
    if (destination) {
      ...
      try {
        fromBech32(receiver);
        this.depositAddress = await this.nomic.generateDepositAddress(
          receiver,
          channel,
          this.wallet.address
        );
  ...
  public getValidator(
    ...
    const stakedValidator = this.stakedValidators.find(
      (validator) => validator.address === address
    );
