...
export class NomicClient implements NomicClientInterface {
  ...
  public async sendToken(to: string, amount: bigint) {
    console.log(
      "🚀 ~ file: nomic-client.ts:245 ~ NomicClient ~ sendToken ~ this.wallet.address:",
      this.wallet.address
    );
    const data = await this.nomic.transfer(this.wallet.address, to, amount);
    console.log(
      "🚀 ~ file: nomic-client.ts:241 ~ NomicClient ~ sendToken ~ data:",
      data
    );
    await this.wallet.sign(data);
  ...
  public async init() {
    if (!this.initialized) {
      await init();
      this.nomic = new OraiBtc(config.restUrl, config.chainId, "bitcoin");
