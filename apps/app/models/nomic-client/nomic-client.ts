...
export class NomicClient implements NomicClientInterface {
  ...
  public async ibcTransferIn(amount: bigint, destinationAddress: string, senderChain: IbcChain, bitcoinAddress: string | undefined) {
  ...
  public async init() {
    if (!this.initialized) {
      await init();
      this.nomic = new OraiBtc(config.restUrl, config.chainId, 'mainnet');
