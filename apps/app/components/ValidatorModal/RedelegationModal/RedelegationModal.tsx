...
import { displayNom, getUrlQueryParam, updateUrlQueryParams } from '@nomic-ui/utils';
...
export const RedelegationModal = observer(({ closeModal }: Props) => {
  ...
  const [redelegationSelection, setRedelegationSelection] = useState<Validator | null>(null);
  ...
  const redelegate = async (fromAddress: string, toAddress: string, amount: bigint) => {
  ...
  return (
    <div className="flex flex-col gap-3">
      <ValidatorCardHeader />
      <RedelegationMenu redelegationSelection={redelegationSelection} setRedelegationSelection={setRedelegationSelection} />
      <ul role="list" className="divide-y divide-gray-200">
        <li key="1" className="py-4">
          <h5 className="text-lg font-semibold">Available for Redelegation</h5>
          <p className="text-sm">{displayNom(selectedValidator ? selectedValidator.amountStaked : BigInt(0)) + ' oraibtc'}</p>
        </li>
      </ul>
      {redelegationSelection ? <StakingInput ctaText={'Amount to Redelegate'} setInput={setRedelegationInput} maxAmount={selectedValidator ? selectedValidator.amountStaked : BigInt(0)} modifier={nomic.modifier} /> : null}
        ...
        <LoadableButton
          ...
          onClick={async () => {
            if (redelegationSelection && selectedValidator) {
              await redelegate(selectedValidator.address, redelegationSelection.address, redelegationInput);
            }
          }}
        />
        <LoadableButton
