...
export const SwapInterface = observer(({ chainBalances, sourceChain, destinationChain, updateBalances, swapChains }: Props) => {
  ...
  useEffect(() => {
    if (document.getElementById('ibc_input') as HTMLInputElement) {
      setInputValue((document.getElementById('ibc_input') as HTMLInputElement).value);
    }
  }, []);
  ...
  const clearTransferState = () => {
    const transferInput = document.getElementById('ibc_input') as HTMLInputElement;
    transferInput.value = '';
  ...
  return (
    <div className="flex flex-col w-full justify-center items-center gap-4 relative text-textPrimary z-10">
      <h3 className="w-full uppercase text-transparent text-md font-medium bg-clip-text bg-gradient-20 from-gradientStart to-gradientStop">Ibc Transfer</h3>
      <TransferEndpoint chain={sourceChain} balance={chainBalances[sourceChain.chainId]} />
      ...
      <TransferEndpoint className="-mt-8" source={false} chain={destinationChain} balance={chainBalances[destinationChain.chainId]} />
      <div className="w-full relative border border-textTertiary rounded-md px-3 py-2 shadow-sm mt-3">
        ...
        <input
          ...
          onChange={(e) => {
            ...
            setTransferAmount(BigInt(Math.floor(Number(e.target.value) * 1e14)));
          }}
        />
        ...
        {!(nomic.wallet instanceof Keplr) ? null : (
          ...
          >
            <div className="flex flex-row items-center justify-center gap-2">
              Fill
              <Image src="/keplr.svg" alt={`keplr logo`} height={24} width={24} />
            </div>
          </button>
        )}
      </div>
      <LoadableButton
        ...
        isActive={!!inputValue && Number(inputValue) > 0 && !!destinationAddress}
        onClick={async () => {
