import { HomeIcon, BanknotesIcon, ArrowsRightLeftIcon, GiftIcon } from '@heroicons/react/24/solid';
    ...
    <div className="fixed p-6 h-screen">
      <div className="w-64 flex flex-col gap-10 bg-surface rounded-lg p-6 min-h-full">
        <div className="flex items-center justify-center w-full min-w-max">
          <Image src={Logo} alt="OraiBtcSubnet" width={128} />
        </div>
        <WalletInfo />
            ...
            <Link href={item.path} key={item.name}>
              <button
                className={classJoin(
                  ...
                  (item.path === '/' && item.path === router.asPath) || (item.path !== '/' && router.asPath.includes(item.path)) ? 'text-textPrimary' : 'text-textSecondary hover:text-textTertiary'
                )}
              >
